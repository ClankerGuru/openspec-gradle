...
abstract class OpenSpecMoveTask : DefaultTask() {
    ...
    init {
        ...
        description = "[tool] Move a class/file to a different package, updating all imports. " +
            ...
            "Chain: Run opsx-find first to check impact."
    }
    ...
    fun move() {
        ...
        val sym = if (symbols.size == 1) symbols.first() else {
            ...
            symbols.firstOrNull { it.kind in setOf(
                zone.clanker.gradle.psi.SymbolKind.CLASS,
                zone.clanker.gradle.psi.SymbolKind.DATA_CLASS,
                zone.clanker.gradle.psi.SymbolKind.INTERFACE,
                zone.clanker.gradle.psi.SymbolKind.ENUM,
                zone.clanker.gradle.psi.SymbolKind.OBJECT,
            ) } ?: symbols.first()
        }
