...
object InstructionsGenerator {
    ...
    private const val MARKER_START_PREFIX = "<!-- OPSX:BEGIN"
    private const val MARKER_END_PREFIX = "<!-- OPSX:END"
    private const val MARKER_START = "$MARKER_START_PREFIX - Generated by OpenSpec Gradle Plugin (https://github.com/ClankerGuru/openspec-gradle) -->"
    private const val MARKER_END = "$MARKER_END_PREFIX - Re-generate with: ./gradlew opsx-sync | Install: https://github.com/ClankerGuru/openspec-gradle -->"
    ...
    fun install(generated: GeneratedFile, projectDir: File, adapter: ToolAdapter) {
        ...
        val startIdx = existing.indexOf(MARKER_START_PREFIX)
        val endIdx = existing.indexOf(MARKER_END_PREFIX)
        val endLineEnd = if (endIdx >= 0) existing.indexOf("-->", endIdx).let { if (it >= 0) it + 3 else -1 } else -1
    ...
    fun clean(projectDir: File, adapter: ToolAdapter): Boolean {
        ...
        val startIdx = content.indexOf(MARKER_START_PREFIX)
        val endIdx = content.indexOf(MARKER_END_PREFIX)
        val endLineEnd = if (endIdx >= 0) content.indexOf("-->", endIdx).let { if (it >= 0) it + 3 else -1 } else -1

        if (startIdx >= 0 && endLineEnd >= 0 && startIdx < endIdx) {
            val before = content.substring(0, startIdx)
            val after = content.substring(endLineEnd)
