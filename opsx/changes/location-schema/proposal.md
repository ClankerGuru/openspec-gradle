...
The current schema ties identity to GitHub naming (`Org/Repo`), making it impossible to include repos from GitLab, Bitbucket, private git servers, or local directories. The `name` field does triple duty — clone URL, directory name, and DSL identifier — which creates coupling that breaks when any of those need to differ.

## Motivation

1. **GitHub lock-in**: The current `"name": "Org/Repo"` format assumes GitHub. Teams using GitLab, Bitbucket, or self-hosted git have no way to use the monolith plugin for those repos.
...
|---------|------|---------|
| Starts with `git@` | Git SSH | `git@gitlab.internal:team/lib.git` |
| Contains `://` | Git URL | `https://github.com/Org/Repo.git` |
| Exactly two segments with `/` | GitHub shorthand | `ClankerGuru/gort` |
| Starts with `/` or `~` | Absolute local path | `/opt/libs/foo` |
| Everything else | Relative local path | `../my-lib` |

## DSL property name derivation

1. If `alias` is present → use it (camelCased)
2. Otherwise → take last path segment of `location`, strip `.git` suffix, camelCase it
