...
The **Wrkx plugin** (`zone.clanker.wrkx`) manages multi-repo workspaces. Declare your repositories in a JSON file, clone them with one command, include them as composite builds with dependency substitution, and manage branches across all repos.
...
Create `~/dev/monolith/workspace.json` with your repositories:
...
| `ref` | string | `"main"` | Branch or tag to check out (used by `wrkx-checkout`) |
...
Each repo from the JSON becomes a camelCase property on the `wrkx` extension:
...
Bracket syntax also works: `wrkx["coreLib"].enable(false)`
...
```bash
./gradlew wrkx-clone                  # dry-run -- preview what will be cloned
./gradlew wrkx-clone -PdryRun=false   # actually clone repos
```
...
| Task | Description |
|---|---|
| `wrkx-clone` | Clone repos from JSON config via `gh` (dry-run by default) |
| `wrkx-checkout` | Checkout configured ref in all enabled repos |
| `wrkx-pull` | Stash, checkout main, pull latest for all enabled repos |
...
|---|---|---|
| `zone.clanker.wrkx.configFile` | `~/dev/monolith/workspace.json` | Path to the JSON config file |
| `zone.clanker.wrkx.repoDir` | `~/dev/monolith` | Base directory for cloned repos |
| `dryRun` | `true` | Set to `false` to actually clone (wrkx-clone only) |

Override via command line:

```bash
./gradlew wrkx-clone \
  -Pzone.clanker.wrkx.repoDir=/path/to/workspace \
  -Pzone.clanker.wrkx.configFile=/path/to/repos.json \
  -PdryRun=false
```

## Composite builds and dependency substitution

When `wrkx.includeEnabled()` is called, the plugin:
...
```bash
./gradlew wrkx-clone -PdryRun=false   # clone all repos
./gradlew wrkx-checkout               # checkout feature-x in all repos
```
...
```bash
./gradlew wrkx-pull                   # sync all repos to latest main when done
```
