...
abstract class MonolithPlugin : Plugin<Settings> {
    ...
    companion object {
        internal fun applyToSettings(project: org.gradle.api.Project, extension: MonolithExtension) {
            ...
            project.tasks.register("opsx-checkout", CheckoutTask::class.java).configure(org.gradle.api.Action {
                it.extensionRepos.addAll(extension.allEntries())
            })

            project.tasks.register("opsx-pull", PullTask::class.java).configure(org.gradle.api.Action {
                it.extensionRepos.addAll(extension.allEntries())
            })

            project.tasks.register("opsx-repos", ReposTask::class.java).configure(org.gradle.api.Action {
                it.outputFile.set(project.layout.projectDirectory.file(".opsx/repos.md"))
                it.extensionRepos.addAll(extension.allEntries())
            })

            project.tasks.register("opsx-clone", CloneTask::class.java).configure(org.gradle.api.Action {
                val home = System.getProperty("user.home")
